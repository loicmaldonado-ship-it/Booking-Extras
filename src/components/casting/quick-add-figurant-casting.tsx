...
import { addFigurantToCastingRole } from "@/lib/casting/actions";

type FigurantOption = { id: string; prenom: string; nom: string };

export function QuickAddFigurantCasting({
  roleId,
  figurants,
  alreadyAddedIds,
}: {
  ...
  function add(figurantId: string) {
    ...
    startTransition(async () => {
      const result = await addFigurantToCastingRole(roleId, figurantId);
      if (result?.error) {
  ...
  if (!open) {
    return (
      <Button type="button" variant="secondary" onClick={() => setOpen(true)}>
        + Ajouter un profil depuis la base
      </Button>
    );
  }
