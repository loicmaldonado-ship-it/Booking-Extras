...
export async function getCastingRoles(projetId: string): Promise<CastingRole[]> {
  ...
  const { data } = await supabase
    .from("casting_roles")
    .select(
      "id, projet_id, nom, date_tournage, date_limite_envoi, categorie_cachet, mode, nb_videos, photo_labels, demande_bande_demo, message_corps, visible_partage, pdf_storage_path, pdf_filename, created_at"
    )
